...
`-AiMermaid` は Claude API への接続が必要なため、**客先環境での使用は原則想定しません**。  
開発者が自環境（PS 7 推奨）で生成した PDF を客先に渡すワークフローを推奨します。

やむを得ず客先で使用する場合は以下を確認してください。

| 確認項目 | 内容 |
|---|---|
| `ANTHROPIC_API_KEY` | 客先環境の環境変数に設定されているか |
| `Set-AiConfig.ps1` | 客先環境で実行済みか |
| スクリプトのエンコーディング | UTF-8 BOM ありで配布しているか（PS 5.1 は BOM なしを Shift-JIS として読む）|
| インターネット接続 | `api.anthropic.com` への HTTPS アクセスが許可されているか |
