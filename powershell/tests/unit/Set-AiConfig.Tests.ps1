...
Describe 'APIキー疎通確認ロジック' {
    ...
    It 'Invoke-RestMethod が成功したらキーを有効と判定できる' {
        Mock Invoke-RestMethod {
            [PSCustomObject]@{ model = 'claude-sonnet-4-6' }
        }
    ...
    It 'Invoke-RestMethod が例外を投げたらキーを無効と判定できる' {
        Mock Invoke-RestMethod { throw [System.Net.WebException]'401 Unauthorized' }
        ...
        try {
            $null = Invoke-RestMethod `
                -Uri     'https://api.anthropic.com/v1/messages' `
                -Method  Post `
                -Headers @{ 'x-api-key' = 'invalid'; 'anthropic-version' = '2023-06-01'; 'content-type' = 'application/json' } `
                -Body    '{}'
        } catch {
            $isValid = $false
        }
