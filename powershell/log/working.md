...
### CheckPsTools に内容比較(SHA256)を追加

- 対象: [tools/CheckPsTools.ps1](../tools/CheckPsTools.ps1) / [README.md](../README.md)
- きっかけ:
  - 従来の ① は `Test-Path` による存在チェックのみで、「配置済みだが内容が古い」状態を
    検出できなかった。スクリプトを修正したあと配置し忘れると、`$PROFILE` 経由で
    呼ばれるのは古い版のままになる(今回の作業中、実際に何度も再配置が必要になった)。
- 変更内容:
  - ① にソースと配置先の SHA256 比較を追加。結果を「未インストール」(赤) と
    「内容不一致」(黄) に分けて表示する。
  - 内容不一致がある場合は、そのまま貼れる再配置コマンド
    `InstallPsScript <相対パス> -c` を提示する(`-c` は前項で追加した複写のみスイッチ)。
  - 戻り値に `StaleCount` / `Stale` を追加。
  - 全件問題なしのときの文言を「全 N 件インストール済み」→「全 N 件インストール済み・最新」に変更。
- 確認:
  - 実装直後に実リポジトリで実行したところ、**CheckPsTools.ps1 自身が「内容不一致」として
    検出された**(編集後に未配置だったため)。提示されたコマンドで再配置し、
    `StaleCount = 0` になることを確認。想定どおりの動作。
